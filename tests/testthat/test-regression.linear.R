test_that("regression.linear.R works", {
    ...
    expect_equal(capture.output(rsq.to.f(0.137931034)),
    ...
    expect_equal(f.to.rsq(0.40, verbose = FALSE),
                 list(f.squared = 0.1600, f = 0.40, r.squared.full = 0.137931034, r.squared.reduced = 0))
    expect_equal(f.to.rsq(0.40, r.squared.full = 0.00862069, verbose = FALSE),
                 list(f.squared = 0.1600, f = 0.40, r.squared.full = 0.00862069, r.squared.reduced = -0.15))
    expect_equal(capture.output(f.to.rsq(0.40)),
                 c("        f.squared                 f    r.squared.full r.squared.reduced ",
                   "         0.160000          0.400000          0.137931          0.000000 "))

    # power.f.regression (= pwrss.f.regression) ------------------------------------------------------------------------
    crrRes <- power.f.regression(r.squared = 0.15, k.total = 3, power = 0.80, verbose = FALSE)
    ...
    expect_equal(crrPty, c("╔══════════════════════════════════════════════════╗",
                           ...
                           "Results",
                           "───────────────────────────────────────────────────",
                           " \033[34m Sample Size       = 66\033[0m\033[1;35m  ◄◄ ",
    ...
    expect_equal(crrRes, pwrss.f.regression(r2 = 0.15, k = 3, power = 0.80, verbose = FALSE))
    expect_equal(crrRes, pwrss.f.regression(f2 = 0.17647059, k = 3, power = 0.80, verbose = FALSE))
    expect_equal(crrRes, pwrss.f.reg(r2 = 0.15, k = 3, power = 0.80, verbose = FALSE))

    crrRes <- power.f.regression(r.squared = 0.15, k.total = 3, power = 0.80, ceiling = FALSE, verbose = FALSE)
    expect_equal(class(crrRes), c("pwrss", "f", "regression"))
    expect_equal(names(crrRes), c("parms", "test", "df1", "df2", "ncp", "null.ncp", "f.alpha", "power", "n"))
    expect_equal(crrRes[["parms"]],
    ...
    crrRes <- power.f.regression(r.squared.change = 0.10, k.total = 5,  k.tested = 2, power = 0.80, verbose = FALSE)
    expect_equal(class(crrRes), c("pwrss", "f", "regression"))
    expect_equal(names(crrRes), c("parms", "test", "df1", "df2", "ncp", "null.ncp", "f.alpha", "power", "n"))
    ...
    crrRes <- power.f.regression(r.squared.change = 0.10, k.total = 5,  k.tested = 2, n = 90, verbose = FALSE)
    expect_equal(class(crrRes), c("pwrss", "f", "regression"))
    expect_equal(names(crrRes), c("parms", "test", "df1", "df2", "ncp", "null.ncp", "f.alpha", "power", "n"))
    expect_equal(crrRes$parms, list(r.squared.change = 0.10, margin = 0, k.total = 5, k.tested = 2, alpha = 0.05,
                                    ceiling = TRUE, verbose = 0, pretty = FALSE))
    ...
    expect_error(power.f.regression(r.squared.change = 0, k.total = 5, k.tested = 2, power = 0.8, verbose = FALSE),
                 "Incorrect value for `r.squared.change`.")
    expect_error(power.f.regression(r.squared.change = 0.99, k.total = 5, k.tested = 2, power = 0.8, verbose = FALSE),
                 "Design is not feasible.")
    expect_error(pwrss.f.regression(r2 = 0.15, f2 = 0.17647059, k = 3, power = 0.80, verbose = FALSE),
                 "Specify either `r2` or `f2`.")

    # power.t.regression (= pwrss.t.regression / pwrss.t.reg) ----------------------------------------------------------
    crrRes <- power.t.regression(beta = 0.20, k.total = 5, r.squared = 0.30, power = 0.80, verbose = FALSE)
    ...
    expect_equal(crrPty, c("╔══════════════════════════════════════════════════╗",
                           ...
                           "Results",
                           "───────────────────────────────────────────────────",
                           " \033[34m Sample Size       = 140\033[0m\033[1;35m  ◄◄ ",
                           "\033[0m  Type 1 Error (α)  = 0.050",
                           "  Type 2 Error      = 0.198",
                           "  Statistical Power = 0.802", ""))
    expect_equal(crrOut, capture.output(power.t.regression(beta = 0.20, k.total = 5, r.squared = 0.30, power = 0.80, verbose = NULL)))
    expect_equal(crrRes, pwrss.t.regression(beta1 = 0.20, k = 5, r2 = 0.30, power = 0.80, verbose = FALSE))
    expect_equal(crrRes, pwrss.t.reg(beta1 = 0.20, k = 5, r2 = 0.30, power = 0.80, verbose = FALSE))

    crrRes <- power.t.regression(beta = 0.20, k.total = 5, r.squared = 0.30, power = 0.80, ceiling = FALSE, verbose = FALSE)
    expect_equal(class(crrRes), c("pwrss", "t", "regression"))
    expect_equal(names(crrRes),
                 c("parms", "test", "std.beta", "std.null.beta", "std.margin", "df", "t.alpha", "ncp", "null.ncp", "power", "n"))
    expect_equal(crrRes[["parms"]],
    ...
    crrRes <- power.t.regression(beta = 0.20, margin = -0.05, alternative = "one.sided", sd.predictor = 0.5, k.total = 5,
                                 r.squared = 0.30, power = 0.80, verbose = FALSE)
    expect_equal(class(crrRes), c("pwrss", "t", "regression"))
    expect_equal(names(crrRes),
                 c("parms", "test", "std.beta", "std.null.beta", "std.margin", "df", "t.alpha", "ncp", "null.ncp", "power", "n"))
    expect_equal(crrRes[["parms"]],
    ...
    expect_equal(names(crrRes),
                 c("parms", "test", "std.beta", "std.null.beta", "std.margin", "df", "t.alpha", "ncp", "null.ncp", "power", "n"))
    expect_equal(crrRes[["parms"]],
                 list(beta = 0, null.beta = 0, margin = c(-0.05, 0.05), sd.predictor = sqrt(2/3 * 1/3), sd.outcome = 1, r.squared = 0.3,
                      k.total = 5, alpha = 0.05, alternative = "two.one.sided", ceiling = TRUE, verbose = 0, pretty = FALSE))
    expect_equal(crrRes[c("test", "std.beta", "std.null.beta", "std.margin", "df", "t.alpha", "ncp", "null.ncp", "power", "n")],
    ...
    crrRes <- power.t.regression(beta = 0, margin = c(-0.05, 0.05), alternative = "two.one.sided", sd.predictor = sqrt(2/3 * 1/3),
                                 k.total = 5, r.squared = 0.30, n = 10792, verbose = FALSE)
    expect_equal(class(crrRes), c("pwrss", "t", "regression"))
    expect_equal(names(crrRes),
                 c("parms", "test", "std.beta", "std.null.beta", "std.margin", "df", "t.alpha", "ncp", "null.ncp", "power", "n"))
    expect_equal(crrRes[["parms"]],
    ...
    expect_error(power.t.regression(beta = 0, margin = 0.05, alternative = "two.one.sided", sd.predictor = 0.5, k.total = 5,
                                    r.squared = 0.30, power = 0.8, verbose = FALSE),
    ...
    expect_error(power.t.regression(beta = 0.20, k.total = 5, r.squared = 0.999, power = 0.80, verbose = FALSE),
                 "Design is not feasible.")
    expect_warning(power.t.regression(beta = 0.20, k.total = 5, r.squared = 0.01, power = 0.80, verbose = FALSE),
