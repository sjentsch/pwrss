...
- prob.matrix:
  ...
  a vector or matrix of cell probabilities under the alternative
  hypothesis

- null.prob.matrix:

  a vector or matrix of cell probabilities under the null hypothesis.
...
- w:
  ...
  Cramer's V. Phi coefficient is defined as `sqrt(X2 / n)` and Cramer's
  V is defined as `sqrt(X2 / (n * v))` where `v` is
